...
<p align="center">
  17 templates &middot; 22 automation scripts &middot; 26 reference guides &middot; 4 OCR profiles &middot; 5 test suites &middot; 0 LaTeX commands required
</p>

> **What this copy contains.** This copy of the skill is text only: the templates in `assets/templates/`, the scripts, the reference guides and the tests. The upstream preview images (`examples/`), the mascot and sponsor artwork, and the resume, thesis, academic-paper, academic-CV and presentation templates with their `references.bib` were not copied, so the gallery below describes those templates without previews. Take them from the upstream repository, ndpvt-web/latex-document-skill, if you need them.
...
<p align="center"><em>Handwritten chaos in, publication-ready PDF out. It's not magic. It's 22 scripts.</em></p>

---

## Template Gallery -- Upstream Templates (17 ship in this copy)
...
Page 1 covers affine and projective space, varieties and morphisms; page 2 covers sheaves, schemes, cohomology and key examples.
...
- `cover-letter.tex` -- Job application
...
```bash
# Install the academic skills for Claude Code, from the repository root
# (INSTALL.md gives the OpenCode and Codex paths)
...
| `pdf-operations.md` | Advanced PDF ops: text extraction, OCR, watermark |
...
Five test suites plus a runner cover the scripts and the shipped templates:
...
bash tests/test_pdf_utils.sh
bash tests/test_analysis_tools.sh
bash tests/test_templates.sh                         # Compiles every shipped template (17)
```
...
Without HappyCapy, this skill would still be a half-baked idea on a sticky note. Instead, it's a full set of templates, scripts and tests, and three capybara mascots with better fashion sense than most academics.
