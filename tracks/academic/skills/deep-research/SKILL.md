---
name: deep-research
description: "Universal deep-research agent team (13 agents, 7 modes: full, brief, paper-review, lit-review, fact-check, Socratic dialogue, systematic review + meta-analysis). Rigorous research, systematic literature search, source verification, cross-source synthesis, APA 7.0 report. Triggers: research, literature review, systematic review, meta-analysis, fact-check, guide my research. For writing the paper itself, see academic-paper."
metadata:
  version: "2.3"
...
=== Phase 4: COMPOSITION ===
     ...
     +-> [report_compiler_agent] -> Full APA 7.0 Draft
         ...
         NOTE: If the user specifies a writing style or points to a
         style-profile file, pass that style (or the file path the user gives)
         to report_compiler_agent and editor_in_chief_agent.
         If no style is given, use the default academic style.
     |
...
При выполнении research pipeline агенты используют MCP-серверы для поиска и извлечения контента. Подробные инструкции — в файлах каждого агента.

> **Опциональность.** Таблица ниже перечисляет исследовательские MCP-серверы (Exa, Jina, Paper Search, Yandex Search) как предпочтительные маршруты. Все они опциональны: если у исследователя эти серверы не установлены, агенты деградируют на встроенные `WebSearch` и `WebFetch` — pipeline остаётся рабочим, меняется только качество/скорость поиска. Имена инструментов ниже — ориентир; используйте фактически доступные в вашей среде.

### Общие правила маршрутизации

| Задача | Основной MCP | Fallback |
|---|---|---|
| Академический/семантический поиск | `mcp__exa__web_search_advanced_exa` | `mcp__paper-search__*` |
| Русскоязычный контент | `mcp__yandex-search__web_search_post` | — |
| Извлечение контента URL | `mcp__jina__read_url` | `mcp__playwright__*` |
| Поиск статей (arXiv) | `mcp__jina__parallel_search_arxiv` | `mcp__paper-search__search_arxiv` |
| Поиск статей (PubMed, bioRxiv) | `mcp__paper-search__*` | `mcp__exa__web_search_exa` |
| Поиск статей (SSRN) | `mcp__jina__parallel_search_ssrn` | `mcp__exa__web_search_exa` |
| BibTeX записи | `mcp__jina__search_bibtex` | — |
| Верификация DOI | `mcp__jina__read_url` (resolve URL) | `mcp__exa__web_search_exa` |
| Проверка даты публикации | `mcp__jina__guess_datetime_url` | — |
| Reranking результатов | `mcp__jina__sort_by_relevance` | — |
| Исследование компаний | `mcp__exa__company_research_exa` | — |

### Правила использования
- `mcp__yandex-search__ai_search_with_yazeka_post` — использовать для сложных запросов (HTTP 202 polling fix applied)
- `mcp__yandex-search__web_search_post` — для простого поиска (предпочтительнее, быстрее)
- Yandex `search_region`: `'ru'` для русскоязычных запросов, `'en'` для английских, `'tr'` для турецких
- Exa free tier: 1000 req/мес — группировать запросы, не дублировать

### Агенты с MCP routing
- `bibliography_agent` — Exa > Paper Search > Yandex (поиск литературы)
...
Handoff data exchange formats between phases are described inline in the "Handoff Protocol" section above.

---

## Integration with Other Skills

This skill is domain-agnostic but can be combined with other academic skills:
...
deep-research + academic-paper-reviewer -> Research then multi-perspective review
deep-research + paper-audit             -> Research then pre-submission audit
```
