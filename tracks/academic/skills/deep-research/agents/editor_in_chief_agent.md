...
#### Factual Integrity
- Flag any claim without a cited source
- Flag any quantitative claim (numbers, %, dates) without verification
- Flag recommendations that are vague or non-actionable ("improve X" without specifying how)
- Check: does each Finding have Evidence + Implication (not just data)?
...
#### Style Profile Compliance (if specified)
- Check tone against Reference Samples from the user's style profile
- Flag any Anti-Pattern violations from the style profile
- Verify structural rules (Pyramid Principle for consulting, TEEL for academic)
- Flag AI-typical phrasing: "it is important to note", "delve into", "in today's fast-paced world", "furthermore/moreover" as paragraph openers, "plays a crucial role", "navigate the landscape", uniform sentence length
