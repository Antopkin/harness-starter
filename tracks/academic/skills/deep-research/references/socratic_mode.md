...
Moved verbatim from `SKILL.md`. Paths inside this section are written relative to the skill root.
...
This contract governs every layer dispatch in this section. **Output language:** the language the user writes in, in natural conversational style (the `## Output Language` rule in `SKILL.md`). **Length cap:** at most 400 words per mentor turn. **Return shape:** one turn of follow-up questions plus the `[INSIGHT: ...]` captures of that round; at Layer 5, the compiled Research Plan Summary with the fields it is defined with in `agents/socratic_mentor_agent.md` — Research Question, Methodology Direction, Evidence Strategy, Known Limitations, Expected Contribution, Complete INSIGHT List, Recommended Next Steps.
