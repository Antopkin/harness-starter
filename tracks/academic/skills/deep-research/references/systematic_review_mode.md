# Systematic Review Mode — PRISMA Pipeline

Moved verbatim from `SKILL.md`. Paths inside this section are written relative to the skill root; the standard `### Checkpoint Rules` referred to below live in `SKILL.md`.
...
This contract governs every phase dispatch in this section. **Output language:** the language the user writes in, academic terminology kept in English (the `## Output Language` rule in `SKILL.md`). **Length cap:** at most 800 words per intermediate agent deliverable (PICOS RQ, protocol, PRISMA flow, RoB assessment, synthesis), or the cap that agent's own definition under `agents/` states where it states one; the Phase 4 and Phase 6 PRISMA report runs to the `systematic-review` band of 5,000-15,000 words. **Return shape:** `templates/prisma_protocol_template.md` for the protocol and `templates/prisma_report_template.md` for the report; for the other agents, the headed fields listed under them above.
