...
> **Нотация.** `<nbsp>` и `<перенос>` в примерах ниже — условные обозначения символов, а не текст для вывода. В готовом тексте вставляй реальный символ: U+00A0 (неразрывный) или U+202F (тонкий неразрывный). Никогда не печатай `<nbsp>`, `\u00A0` или `&nbsp;` как литералы (`&nbsp;` — только в HTML).
