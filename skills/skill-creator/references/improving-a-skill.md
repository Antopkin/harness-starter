...
The full Improve-mode procedure: setup, the iteration loop (execute, grade, blind compare, analyse, update, re-version), the final report, and the reduced-rigor path without subagents. Referenced from `SKILL.md`.
...
3. **Verify or create evals**:
   ...
   - Use `scripts/init_json.py evals evals/evals.json` to create with correct structure

4. **Create tasks** for baseline:

   The baseline is three runs of v0 on the same eval. Create one run directory per run — `workspace/v0/runs/run-1/` through `run-3/` — and stage the eval prompt and any test files into each before spawning anything. Those directories are the record of the three tasks.
...
**Output language:** the language of the eval prompt. **Length cap:** at most 200 words per run in the executor's returned text. **Return shape:** the paths written — `transcript.md`, `outputs/`, `metrics.json`, `user_notes.md` — plus any blocking issue.
...
**Output language:** English. **Length cap:** at most 200 words in the grader's returned text, with each expectation's evidence under 50 words. **Return shape:** `grading.json` at the given path, including `user_notes_summary` and `eval_feedback`, per `references/schemas.md`.
...
**Output language:** English. **Length cap:** at most 300 words per comparator. **Return shape:** the `comparison.json` fields — winner (A or B), rubric scores per output, reasoning, assertion evidence — as defined in `agents/comparator.md` and `references/schemas.md`.
...
**Output language:** English. **Length cap:** at most 400 words. **Return shape:** the `analysis.json` fields — winner strengths, loser weaknesses, instruction-following observations, and prioritised improvement suggestions — as defined in `agents/analyzer.md` and `references/schemas.md`.
...
**Output language:** the language of the user's request. **Length cap:** at most 400 words. **Return shape:** the four fields above — best version, score progression, key improvements, recommendation — plus the path the best skill was copied to.
