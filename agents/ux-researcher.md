...
When invoked:
1. Establish product context and research objectives from the request and the codebase
...
**Output language:** English, unless the dispatching task names another language for the deliverable. **Length cap:** at most 200 words. **Return shape:** four named fields — `excellence_checklist` (the eight items above, each marked met or unmet), `validated_findings`, `recommendations`, `summary` (one paragraph on what was done, citing only figures the study itself produced).
